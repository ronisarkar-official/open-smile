...
const podiumConfig: Record<number, { order: string; height: string; bg: string; shadow: string; badge: typeof Crown }> = {
  1: { order: "order-2", height: "min-h-72 sm:min-h-80", bg: "bg-primary", shadow: "shadow-[8px_8px_0_#000]", badge: Crown },
  2: { order: "order-1", height: "min-h-56 sm:min-h-64 mt-auto", bg: "bg-accent", shadow: "shadow-[5px_5px_0_#000]", badge: Medal },
  3: { order: "order-3", height: "min-h-48 sm:min-h-56 mt-auto", bg: "bg-secondary", shadow: "shadow-[5px_5px_0_#000]", badge: Medal },
};
  ...
  return (
    <main id="main-content" className="mx-auto w-full max-w-[1280px] px-2 pb-8 pt-6 sm:px-4 sm:pt-10">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex border-[3px] border-black bg-card">
        ...
        <div className="grid grid-cols-3 items-end gap-3 sm:gap-5">
          {podium.map((user) => {
            ...
            return (
              <article
                key={user.rank}
                className={`${config.order} ${config.height} ${config.bg} flex flex-col items-center justify-end border-[3px] border-black p-4 sm:p-6 ${config.shadow}`}
              >
                <div className="relative">
                  <Avatar className="size-14 border-[3px] border-black sm:size-16">
                    <AvatarFallback className="bg-card text-lg font-bold sm:text-xl">{user.avatar}</AvatarFallback>
                  </Avatar>
                  {user.rank === 1 && (
                    <div className="absolute -right-2 -top-2 flex size-7 items-center justify-center border-[2px] border-black bg-primary">
                ...
                <div className="mt-3 inline-flex items-center gap-1 border-[2px] border-black bg-card px-2 py-1 font-mono text-[10px] font-bold">
      ...
      <section className="mt-8" aria-label="Full rankings">
        <div className="border-[3px] border-black bg-card shadow-[5px_5px_0_#000]">
          <div className="grid grid-cols-[auto_1fr_auto] items-center gap-4 border-b-[3px] border-black bg-muted px-4 py-3 sm:grid-cols-[auto_1fr_auto_auto] sm:px-6">
            ...
            <div
              ...
              className={`grid grid-cols-[auto_1fr_auto] items-center gap-4 border-b border-black/10 px-4 py-3 transition-colors sm:grid-cols-[auto_1fr_auto_auto] sm:px-6 ${
                "isCurrentUser" in user && user.isCurrentUser
                  ? "bg-primary/20 border-b-[2px] border-black/20"
                  : "hover:bg-muted/50"
              }`}
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-bold">
                  {user.name}
                  {"isCurrentUser" in user && user.isCurrentUser && (
                    <span className="ml-2 inline-flex border-[2px] border-black bg-primary px-1.5 py-0.5 text-[10px] font-bold uppercase">You</span>
                  )}
                </span>
              </div>
