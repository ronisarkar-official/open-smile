...
export async function verifyOtpAttemptAllowed(email: string): Promise<{
	allowed: boolean;
	attemptsRemaining: number;
}> {
	if (!process.env.DATABASE_URL) return { allowed: true, attemptsRemaining: MAX_OTP_ATTEMPTS };
	const otp = await findOtpCode(email);
	if (!otp) return { allowed: true, attemptsRemaining: MAX_OTP_ATTEMPTS };
	if (otp.attempts >= MAX_OTP_ATTEMPTS) {
		await deleteOtpCode(email);
		return { allowed: false, attemptsRemaining: 0 };
	}
	return { allowed: true, attemptsRemaining: MAX_OTP_ATTEMPTS - otp.attempts };
}
